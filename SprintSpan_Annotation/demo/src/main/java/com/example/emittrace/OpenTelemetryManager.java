...
public final class OpenTelemetryManager {
        ...
        public static OpenTelemetry initOpenTelemetry() {
                ...
                OtlpGrpcSpanExporter spanExporter = OtlpGrpcSpanExporter.builder().setTimeout(2, TimeUnit.SECONDS)
                                .build();
                BatchSpanProcessor spanProcessor = BatchSpanProcessor.builder(spanExporter)
                                .setScheduleDelay(100, TimeUnit.MILLISECONDS).build();

                Resource serviceName = Resource
                                .create(Attributes.of(AttributeKey.stringKey("service.name"),
                                                "TraceSpanService"));
                // SdkTracerProvider tracerProvider =
                // SdkTracerProvider.builder().addSpanProcessor(spanProcessor)
                // .setResource(OpenTelemetrySdkAutoConfiguration.getResource().merge(serviceName))
                // .build();
